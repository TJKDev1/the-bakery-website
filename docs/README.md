# The Bakery Website Documentation

Welcome to The Bakery website documentation. This guide covers everything you need to know about the project structure, design system, and how to make changes.

## Table of Contents

- [Project Overview](./project-overview.md)
- [File Structure](./file-structure.md)
- [Design System](./design-system.md)
- [Components Guide](./components.md)
- [Adding Content](./adding-content.md)
- [Coding Standards](./coding-standards.md)
- [Deployment](./deployment.md)

## Quick Start

1. Clone the repository
2. Run `npx serve .` to start a local server
3. Open `http://localhost:3000` in your browser

## Links

- **Live Site**: <https://theofficialbakery.github.io/the-bakery-website/>
- **Redbubble Shop**: <https://www.redbubble.com/people/the-bakery-shop/>
- **TikTok**: <https://www.tiktok.com/@thebakerytiktok>
- **Email**: <TheOfficialBakery@proton.me>
